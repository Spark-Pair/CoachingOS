...
function Modal({ title, isOpen, onClose, children, footer, className = '', disableClose = false }) {
  ...
  }, [disableClose, onClose, shouldRender])
    ...
    <div className={`modal-layer${isClosing ? ' closing' : ''}`} role="presentation">
      ...
      <section className={`modal-panel${className ? ` ${className}` : ''}`} role="dialog" aria-modal="true" aria-labelledby="modal-title">
        <div className="modal-header">
          ...
          <button type="button" className="icon-button" onClick={onClose} aria-label="Close modal" disabled={disableClose}>
            <X size={18} />
          </button>
        </div>
